...
Liquidations are the automatic transfer of collateral from an insufficiently collateralized vault, along with the transfer of that vault's debt, to the Maker protocol. Auctions are therefore used to sell the transfered collateral for USDV in an attempt to cancel out the debt now assigned to the protocol.

This is achieved through Liquidations2.0, which is a Dutch-style auction system, where auction prices start high and drop over time. This auction system uses USDV for all collateral purchases, meaning that as time passes, the USDV that the protocol is willing to accept for the offered collateral will also fall.
...
By clicking on the Deposit/Withdraw button on the homepage header, the user will be asked to conduct two one-time actions to **Unlock USDV to bid**:

<img src="https://i.imgur.com/aO7HrUE.png" alt="unlock usdv" width="400" />

This modal requires two transactions; 1. Unlock Usdv in the VAT - which authorizes the user's address to deposit into the VAT. 2. Authorize the VAT - which authorizes the VAT to use the USDV that the user will deposit.

Note: The VAT contract is the core vault engine of the Maker Protocol and as such manages the central accounting invariants of USDV. Depositing and interacting with the VAT is therefore necessary in order to participate in auctions. More technical information about the VAT can be found [here](https://docs.makerdao.com/smart-contract-modules/core-module/vat-detailed-documentation#2-contract-details).

# Deposit Usdv to Bid

Once the above modal transactions are confirmed, the user will be able specify an amount of USDV to deposit from their wallet into the VAT.

<img src="https://i.imgur.com/XKhzmFv.png" alt="unlock usdv" width="400" />

The USDV that the user has deposited into the VAT will then be available for auctions. Following this step, the user can proceed to the collateral-specific page.
...
<img src="https://i.imgur.com/j7eFAID.png" alt="unlock usdv" width="700" />
...
<img src="https://i.imgur.com/FQxKcby.png" alt="unlock usdv" width="400" />

Approving this one-time authorization will then reveal the Auction bid modal allowing the user to specify the amount of USDV they wish to offer and the equivalent collateral that this amount of USDV will purchase.

<img src="https://i.imgur.com/biLZw2y.png" alt="unlock usdv" width="400" />
...
<img src="https://i.imgur.com/h5YfcpG.png" alt="unlock usdv" width="700" />
