...
export function useSystemStats(): any {
  ...
  const { data: totalUsdv, error: totalUsdvError } = useGlobalMax();

  // return data needed for each field in fieldMap and let format function do the rest
  // ['Active Auctions', 'Inactive Auctions', 'Vaults requiring kick', 'Usdv required for Auctions', 'Global max available']
  const data =
    auctions && unsafeVaults && totalUsdv ? [auctions, auctions, unsafeVaults, auctions, totalUsdv] : null;
  const error = auctionsError || unsafeVaultsError || totalUsdvError;
...
export function useSystemStatsSidebar(ilk: string): any {
  ...
  // return data needed for each field in fieldMap and let format function do the rest
  // ['Undercollateralized Vaults', 'Active Auctions', 'Inactive Auctions', 'Usdv required for Auctions', 'Limit per collateral available']
